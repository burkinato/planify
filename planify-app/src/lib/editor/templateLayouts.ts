...
export function buildRegions(style: string): TemplateRegion[] {
  ...
  if (style === 'premiumAudit') return [
    ...
    r('emergency',       'emergency',   '112 Acil Durum',        72,  13, 26, 10, 'red'),
    r('instructions',    'instruction', 'Talimatlar',            72,  25, 26, 22, 'green'),
    r('legend',          'legend',      'Lejand / Semboller',    72,  49, 26, 18, 'info'),
    r('assembly',        'assembly',    'Vaziyet Planı',         72,  69, 26, 10, 'blue'),
  ...
  if (style === 'sitePremium') return [
    ...
    r('assembly',        'assembly',    'Vaziyet / Toplanma',    68,  13, 30, 30, 'blue'),
    r('emergency',       'emergency',   '112 Acil Yardım',       68,  45, 30, 10, 'red'),
    r('legend',          'legend',      'Lejand',                68,  57, 30, 18, 'info'),
    r('instructions',    'instruction', 'Acil Durum Talimatı',    2,  78, 45, 18, 'green'),
    r('approval',        'approval',    'Onay ve Revizyon',      49,  78, 49, 18, 'neutral'),
  ];
  if (style === 'corporate3d') return [
    r('header',          'header',      HEADER_TITLE,            2,   2, 96, 10, 'green'),
    r('emergency',       'emergency',   '112 Acil Hattı',         2,  14, 18, 10, 'red'),
    r('instructions',    'instruction', 'Talimatlar',             2,  26, 18, 29, 'green'),
    r('legend',          'legend',      'Lejand',                 2,  57, 18, 20, 'info'),
  ...
  if (style === 'panoramic') return [
    ...
    r('legend',          'legend',      'Lejand',                80,  14, 18, 32, 'info'),
    r('instructions',    'instruction', 'Talimatlar',            80,  48, 18, 22, 'green'),
  ...
  if (style === 'leftRail') return [
    ...
    r('legend',          'legend',      'Lejand',                 3,  80, 44, 17, 'info'),
    r('assembly',        'assembly',    'Toplanma Alanı',        50,  80, 22, 17, 'blue'),
    r('approval',        'approval',    'Onay / Revizyon',       75,  80, 22, 17, 'neutral'),
  ];
  if (style === 'rightRail') return [
    r('header',          'header',      HEADER_TITLE,            3,   3, 94,  9, 'green'),
    r('drawing',         'drawing',     'Ana Çizim Alanı',        3,  15, 70, 65, 'paper'),
    r('legend',          'legend',      'Lejand',                76,  15, 21, 35, 'info'),
  ...
  if (style === 'bottomBand') return [
    ...
    r('legend',          'legend',      'Lejand',                67,  77, 15, 20, 'info'),
    r('approval',        'approval',    'Onay / Revizyon',       84,  77, 13, 20, 'neutral'),
  ];
  if (style === 'dualRails') return [
    ...
    r('legend',          'legend',      'Lejand',                 3,  57, 17, 38, 'info'),
    r('assembly',        'assembly',    'Toplanma Alanı',        22,  79, 35, 18, 'blue'),
    r('approval',        'approval',    'Onay ve Revizyon',      59,  79, 38, 18, 'neutral'),
  ];
  return [
    r('header',          'header',      HEADER_TITLE,            3,   3, 94, 10, 'green'),
