...
export function DashboardMetrics({ projects }: DashboardMetricsProps) {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center divide-y md:divide-y-0 md:divide-x divide-surface-600/30 mb-8 px-1">
      {metrics.map((metric) => (
        <div
          key={metric.label}
          className="flex-1 w-full px-6 py-4 flex flex-col group hover:bg-surface-900/40 transition-colors"
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-surface-900 rounded-xl border border-surface-600/40 shadow-sm">
                {metric.icon}
              </div>
              <p className="text-[10px] font-black text-surface-400 uppercase tracking-[0.2em]">{metric.label}</p>
            </div>
            <h4 className="text-4xl font-black tracking-tighter text-surface-100">{metric.value}</h4>
          </div>
          <div className="flex items-center gap-2">
            <div className={`w-1.5 h-1.5 rounded-full ${metric.dotColor} shadow-sm`} />
            <p className="text-[9px] font-bold text-surface-400 uppercase tracking-widest">
              {metric.description}
            </p>
          </div>
        </div>
