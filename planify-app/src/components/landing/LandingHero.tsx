...
function EditorPreview() {
  return (
    <div className="relative animate-landing-float w-full max-w-[600px] mx-auto z-10">
      ...
      <div className="absolute -inset-1 bg-gradient-to-r from-primary-500 to-indigo-500 rounded-[1.5rem] blur-2xl opacity-20" />
      
      <div className="relative rounded-2xl shadow-2xl shadow-primary-900/5 border border-slate-200/60 bg-white/90 backdrop-blur-xl overflow-hidden ring-1 ring-slate-900/5">
        {/* Browser chrome */}
        ...
        <div className="flex" style={{ height: 340 }}>
          ...
          <div className="w-14 bg-slate-900 flex flex-col items-center py-4 gap-3 shrink-0">
            {['W','D','P','S','⬡'].map((t, i) => (
              <div key={i} className={`w-9 h-9 rounded-xl flex items-center justify-center text-[12px] font-bold cursor-pointer transition-all duration-300 ${i === 0 ? 'bg-blue-600 text-white shadow-lg shadow-primary-500/30' : 'text-slate-400 hover:bg-slate-700 hover:text-white'}`}>
                {t}
              </div>
            ))}
          </div>
          ...
          <div className="flex-1 bg-[#F8FAFC] relative overflow-hidden" style={{ backgroundImage: 'radial-gradient(circle at 1px 1px,#cbd5e1 0.8px,transparent 0)', backgroundSize: '24px 24px' }}>
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 380 340" fill="none">
              ...
              <rect x="157" y="115" width="6" height="75" fill="rgba(37,99,235,0.15)" stroke="var(--color-primary-500)" strokeWidth="1.5" strokeDasharray="4 2" />
              <circle cx="160" cy="115" r="4" fill="var(--color-primary-500)" className="animate-pulse" />
              <circle cx="160" cy="190" r="4" fill="var(--color-primary-500)" className="animate-pulse" />
            </svg>
          </div>
          {/* Right panel */}
          <div className="w-52 bg-white/95 backdrop-blur-md border-l border-slate-200 p-4 shrink-0 flex flex-col">
            ...
            <div className="mt-4 pt-4 border-t border-slate-100">
              ...
              <div className="flex gap-2">
                {['#0F172A', 'var(--color-primary-500)', '#10B981', '#F59E0B'].map((c, i) => (
...
export default function LandingHero() {
  ...
  return (
    <>
      {/* HERO */}
      <section className="relative pt-32 pb-20 px-6 overflow-hidden bg-[#FAFAFC] selection:bg-primary-100 selection:text-primary-900">
        {/* Background Mesh Gradient */}
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-primary-100/60 blur-[100px]" />
          ...
          <div className="space-y-10 animate-fade-up z-10 max-w-2xl">
            ...
            <div className="flex items-center gap-8 pt-6 border-t border-slate-200/60">
              <div className="flex -space-x-3">
                ...
                <div className="w-10 h-10 rounded-full border-2 border-white bg-primary-50 flex items-center justify-center text-[10px] font-bold text-primary-600 shadow-sm z-10">
                  +500
                </div>
              </div>
              <div>
